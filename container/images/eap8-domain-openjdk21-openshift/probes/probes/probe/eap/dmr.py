        ...
        super(ServerStatusTest, self).__init__(
            {
                ...
                "name": "server-state",
                "address": {
                    "host": os.getenv('JBOSS_EAP_DOMAIN_HOST_NAME'),
                    "server": os.getenv('JBOSS_EAP_DOMAIN_SERVER_NAME')
                }
            }
        )
    ...
    def evaluate(self, results):
        ...
        for result in results["result"]:
            if result["outcome"] != "success" and result.get("failure-description"):
                ...
                messages[result["address"][2]["deployment"]] = "DMR query failed"
            else:
                deploymentStatus = result["result"]
                messages[result["address"][2]["deployment"]] = deploymentStatus
